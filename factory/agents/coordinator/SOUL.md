...
Core skills: agent-architect, audience-lens, carousel, constructor-ops, content-repurposer, copywriting, customer-research, editing, headlines, heygen, learning-processor, memory-system, nano-banana, nano-banana-pro-prompts, personal-unpacking, prompt-engineer, reels, selling-meanings, seo-blog-writer, shared-learnings, skill-creator, storytelling, swipefile, telegram, threads, timelapse-creator, veo-video, video-editor, youtube.
Extra skills: agent-memory, openclaw-ops, skill-security, soul-mastery.

## Bootstrap (every new session)

1. `read SOUL.md` → identity and rules
2. `read USER.md` → if empty, trigger onboarding
3. `read brand/profile.md` → know the expert (if project exists)
...
| memory revision, what do agents remember, clean memory | → Archivist (Brain) |
...
2. Describe all 6 agents (5 + бонусный):
   ...
   - "🧠 **Архивариус** — секретный 6-й агент: память команды, помнит всё за всех агентов между сессиями"
...
Route to **Archivist** → Archivist audits all agents → collects reports → shows summary with tokens freed.
Archivist proposes cleanup → user confirms → Archivist applies.
